...
class KalmanFilter:
    ...
    def __init__(self, initial_observation, v=307200, dth=1000, k=0, q=1e-5, r=0.01, nmiss=3, pd=0.9):
        ...
        x = np.atleast_2d(np.array(initial_observation, dtype=float)).T  # ✅ Solution robuste
        ...
        self.__eucl_max_dist = 10  # ou 15, à tester selon ton image
        self.__d_th = dth
    ...
    def update(self, z):
        ...
        if z is None:
            ...
            x = np.atleast_2d(np.array(z, dtype=float)).T

            mu = self.__xhat
            sigma = self.__P + self.__Q
            # Euclidean gating
            eucl_dist = np.linalg.norm((x - mu))
            if eucl_dist > self.__eucl_max_dist:
                return True
